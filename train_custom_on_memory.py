...
x_train = np.empty((train_size, shape[0], shape[1], shape[2]))
...
for idx, (name, label) in tqdm(enumerate(zip(train_df['Id'], train_df['target_vec']))):
    ...
    x_train[idx] = load_4ch_image(path, shape)/255.
    y_train[idx][label] = 1

x_valid = np.empty((valid_size, shape[0], shape[1], shape[2]))
...
for idx, (name, label) in tqdm(enumerate(zip(valid_df['Id'], valid_df['target_vec']))):
    ...
    #print(idx, name, label)
    path = os.path.join('./data/train/', name)
    x_valid[idx] = load_4ch_image(path, shape)/255.
    y_valid[idx][label] = 1
